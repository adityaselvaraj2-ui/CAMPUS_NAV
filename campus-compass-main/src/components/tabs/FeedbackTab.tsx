...
const FeedbackTab = ({ campus }: FeedbackTabProps) => {
  ...
  const [liveAvgMood, setLiveAvgMood] = useState<number>(4.1);
  const [liveBuildingRankings, setLiveBuildingRankings] = useState(buildingRankings);
  const [liveWordCloud, setLiveWordCloud] = useState(wordCloudData);
  const [loadingAnalytics, setLoadingAnalytics] = useState(false);

  const moodIdx = Math.min(Math.round(moodValue / 5 * 5), 5);
  const overallScore = liveAvgMood;
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/feedback/${encodeURIComponent(campus.name)}`);
      const data = await res.json();
      if (data.success) {
        setLiveCount(data.count);
        if (data.avgMood) setLiveAvgMood(parseFloat(data.avgMood));
        if (data.buildingRankings?.length) setLiveBuildingRankings(data.buildingRankings);
        if (data.wordCloud?.length) {
          setLiveWordCloud(data.wordCloud.map((w: { word: string; count: number }) => ({
            word: w.word,
            count: w.count,
  ...
  const sortedRankings = useMemo(() =>
    [...liveBuildingRankings].sort((a, b) => b.score - a.score),
    [liveBuildingRankings]
  );
        ...
        <div className="flex-1 glass-card p-6">
          ...
          {liveCount > 0 && (
            <div className="mb-4 flex items-center gap-2 animate-fade-up">
              <span className="px-3 py-1 rounded-full bg-primary/10 border border-primary/20 font-ui text-[10px] tracking-widest text-primary">
                📊 {liveCount} student{liveCount !== 1 ? 's' : ''} have shared feedback
              </span>
              {loadingAnalytics && <span className="text-[9px] text-text-3 font-ui tracking-widest">UPDATING...</span>}
            </div>
          )}
